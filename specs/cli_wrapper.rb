...
class TestCLIWrapper < Minitest::Test
  ...
  def test_post_screenshot_with_ignore_region_null
    ...
    assert_equal(
      @cli_wrapper.post_screenshots('some-name', {}, [Tile.new(10, 10, 20, 20, filepath:'some-file-path')], 'some-debug-url'),
  ...
  def test_post_screenshot_with_ignore_region_present
    ...
    assert_equal(
      @cli_wrapper.post_screenshots('some-name', {}, [Tile.new(10, 10, 20, 20, filepath:'some-file-path')], 'some-debug-url', @ignored_elements_data),
      {'link' => 'snapshot-url-link', 'success' => true}
    )
  end
  ...
  def test_request_body
    ...
    name = "some-name"
    debug_url = "debug-url"
    response = @cli_wrapper.request_body(name, tag, [tile], debug_url, @ignored_elements_data, @considered_elements_data)
  ...
  def test_request_body_when_optional_values_are_null
    ...
    response = @cli_wrapper.send(:request_body, name, tag, [tile], debug_url, ignored_elements_data, considered_elements_data)
    assert_equal response["name"], name
    assert_equal response["external_debug_url"], debug_url
    assert_equal response["tag"], tag
    assert_equal response["tiles"], [tile.to_h]
